class OrderFormViewModel {
	...
	_completeOrder(token){
		...
		async.waterfall([
				...
				this._determineUpdateOrCreate.bind(this)
			],
			possibleError =>{
				if(possibleError){
	...
	_determineUpdateOrCreate(nullOrUser, callback){
		if(nullOrUser){
			this._updateUserData()
				.then(() => callback())
				.fail(error => callback(error));
		} else {
			var tmpUser = this._makeTempUserSchema();
			this.storageHelper.LoggedInUser = tmpUser;
			this.webSvc.CreateUser(tmpUser)
				.then(() => callback())
	...
	_checkIfUserExists(callback){
		if(!this.storageHelper.LoggedInUser){
			this.webSvc.GetUserByEmail(this.email())
				.then((possibleUser)=> callback(null, possibleUser))
				.fail(error => callback(error));
		} else {
			callback(this.storageHelper.LoggedInUser);
		}
	}
